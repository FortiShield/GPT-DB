...
// Utility function to transform gptdb-fs:// URLs to HTTP service URLs
export const transformFileUrl = (url: string): string => {
  try {
    if (!url.startsWith('gptdb-fs://')) {
      return url;
    }

    // Parse the gptdb-fs:// URL structure
    const parsedUrl = new URL(url);

    if (parsedUrl.protocol !== 'gptdb-fs:') {
      return url;
    }
