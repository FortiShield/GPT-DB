...
from gptdb.core.awel.util.cache_util import FileCacheStorage
from gptdb.core.operators import PromptBuilderOperator, RequestBuilderOperator
from gptdb.model import AutoLLMClient
...
其中 po 文件的部分内容如下：
```
#: ../gptdb/storage/vector_store/chroma_store.py:21
msgid "Chroma Vector Store"
msgstr ""
```

你的任务是将 msgstr 的内容翻译成{language}, 切记，不能对 msgid 进行任何修改，也不能对文件标识（如：#: ../gptdb/storage/vector_store/chroma_store.py:21）进行任何修改。
...
class ModuleInfo(NamedTuple):
    ...
    base_module: str  # Base module name (e.g., gptdb)
...
class BatchOperator(JoinOperator[str]):
    ...
    async def batch_run(self, blocks: List[str], ext_dict: Dict[str, Any]) -> str:
        ...
        # ~/.cache/gptdb/i18n/cache
        cache_dir = Path.home() / ".cache" / "gptdb" / "i18n" / "cache"
            ...
            for input_data, out_data in results:
                ...
                if count_msgstr != count_out_msgstr:
                    ...
                    outfile = os.path.join(
                        "/tmp", f"gptdb_i18n_{model_name}_{language}"
                    )
...
if __name__ == "__main__":
    from gptdb.configs.model_config import ROOT_PATH
    from gptdb.util.utils import setup_logging
    ...
    setup_logging("gptdb", default_logger_level=log_level)
