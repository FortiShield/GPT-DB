...
docker run -it --rm -e SILICONFLOW_API_KEY=${SILICONFLOW_API_KEY} \
 -p 5670:5670 --name gptdb khulnasoft/gptdb-openai
```
Or with AI/ML API:
```bash
...
Create a `toml` file named `gptdb-local-gpu.toml` and add the following content:
...
docker run --ipc host --gpus all \
  -it --rm \
  -p 5670:5670 \
  -v ./gptdb-local-gpu.toml:/app/configs/gptdb-local-gpu.toml \
  -v ./models:/app/models \
  --name gptdb \
  khulnasoft/gptdb \
  gptdb start webserver --config /app/configs/gptdb-local-gpu.toml
```

#### Explanation of the Command:
- `--ipc host`: Enables host IPC mode for better performance.
- `--gpus all`: Allows the container to use all available GPUs.
- `-v ./gptdb-local-gpu.toml:/app/configs/gptdb-local-gpu.toml`: Mounts the local configuration file into the container.
- `-v ./models:/app/models`: Mounts the local `models` directory into the container.
- `khulnasoft/gptdb`: The Docker image to use.
- `gptdb start webserver --config /app/configs/gptdb-local-gpu.toml`: Starts the webserver with the specified configuration file.
...
2. Modify the `gptdb-local-gpu.toml` configuration file to point to the correct paths:

   ```toml
   [service.web.database]
   type = "sqlite"
   path = "/app/pilot/message/gptdb.db"
   ```
