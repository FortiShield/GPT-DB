...
1. Open-source Text2SQL dataset repository by Ant Group: [Falcon](https://github.com/khulnasoft/Falcon)
2. GPT-DB supports LLM evaluation based on the Falcon benchmark dataset

# Introduction

To objectively and fairly evaluate models on Text2SQL tasks, we provide a benchmarking module and dataset. This module supports comprehensive evaluation of all models in the GPT-DB framework and provides an evaluation report.

The benchmark dataset used by the module, [Falcon](https://github.com/khulnasoft/Falcon), is a high-quality and evolving open-source Text2SQL dataset from Ant Group.
...
- Step1: Upgrade to V0.7.4 and upgrade the metadata database
    ...
    For SQLite, the table schema is upgraded automatically by default. For MySQL, you need to run the DDL manually. The file assets/schema/gptdb.sql contains the complete DDL for the current version. Version-specific DDL changes can be found under assets/schema/upgrade. For example, if you are upgrading from v0.7.1 to v0.7.4, you can run the following DDL:
 
    ```
    mysql -h127.0.0.1 -uroot -p{your_password} < assets/schema/upgrade/v0_7_4/upgrade_to_v0.7.4.sql
    ```

- Step2: Start the GPT-DB service, and wait for the benchmark dataset to load automatically. When you see the log line, the dataset has finished loading (about 1~5 minute).
