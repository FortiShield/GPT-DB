...
We are thrilled to announce that GPT-DB now supports inference with the Qwen3 series models!
...
To run GPT-DB with the local Qwen3 model. You can provide a configuration file to specify the model path and other parameters.
Here is an example configuration file `configs/gptdb-local-qwen3.toml`:
...
```bash
uv run gptdb start webserver --config configs/gptdb-local-qwen3.toml
```

Open your browser and visit `http://localhost:5670` to use the Qwen3 models in GPT-DB.

Enjoy the power of Qwen3 in GPT-DB!
