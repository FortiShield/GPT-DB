...
from gptdb.util.tracer import SpanType, SpanTypeRunName, initialize_tracer, root_tracer
from gptdb.util.utils import (
    logging_str_to_uvicorn_level,
    setup_http_service_logging,
    setup_logging,
)
from gptdb_app.base import (
...
from gptdb_app.component_configs import initialize_components
from gptdb_app.config import ApplicationConfig, ServiceWebParameters, SystemParameters
from gptdb_serve.core import add_exception_handler
...
app = create_app(
    title=_("GPT-DB Open API"),
    description=_("GPT-DB Open API"),
    version=version,
    openapi_tags=[],
)
...
def mount_routers(app: FastAPI):
    ...
    from gptdb_app.knowledge.api import router as knowledge_router
    from gptdb_app.openapi.api_v1.api_v1 import router as api_v1
    from gptdb_app.openapi.api_v1.editor.api_editor_v1 import (
        router as api_editor_route_v1,
    )
    from gptdb_app.openapi.api_v1.feedback.api_fb_v1 import router as api_fb_v1
    from gptdb_app.openapi.api_v2 import router as api_v2
    from gptdb_serve.agent.app.controller import router as gpts_v1
    from gptdb_serve.agent.app.endpoints import router as app_v2
    ...
    from gptdb_serve.agent.app.recommend_question.controller import (
...
def mount_static_files(app: FastAPI, param: ApplicationConfig):
    if param.service.web.new_web_ui:
        static_file_path = os.path.join(ROOT_PATH, "src", "gptdb_app/static/web")
    else:
        static_file_path = os.path.join(ROOT_PATH, "src", "gptdb_app/static/old_web")
...
def initialize_app(param: ApplicationConfig, args: List[str] = None):
    ...
    from gptdb.model.cluster import initialize_worker_manager_in_client
    ...
    setup_logging(
        "gptdb",
        log_config,
        default_logger_filename=os.path.join(LOGDIR, "gptdb_webserver.log"),
    )
    ...
    if not web_config.light:
        from gptdb.model.cluster.storage import ModelStorage
        from gptdb_serve.model.serve import Serve as ModelServe
...
def run_webserver(config_file: str):
    ...
    trace_file = trace_config.file or os.path.join(
        "logs", "gptdb_webserver_tracer.jsonl"
    )
    ...
    initialize_tracer(
        trace_file,
        system_app=system_app,
        root_operation_name=trace_config.root_operation_name or "GPT-DB-Webserver",
    ...
    with root_tracer.start_span(
        ...
        # TODO
        from gptdb_serve.agent.agents.expand.app_start_assisant_agent import (  # noqa: F401
            StartAppAssistantAgent,
        )
        from gptdb_serve.agent.agents.expand.intent_recognition_agent import (  # noqa: F401
...
def scan_configs():
    from gptdb.model import scan_model_providers
    from gptdb_app.initialization.app_initialization import scan_app_configs
    from gptdb_app.initialization.serve_initialization import scan_serve_configs
    from gptdb_ext.storage import scan_storage_configs
    from gptdb_serve.datasource.manages.connector_manager import ConnectorManager
...
def load_config(config_file: str = None) -> ApplicationConfig:
    ...
    from gptdb.configs.model_config import ROOT_PATH as GPTDB_ROOT_PATH

    if config_file is None:
        config_file = os.path.join(
            GPTDB_ROOT_PATH, "configs", "gptdb-proxy-siliconflow.toml"
        )
    elif not os.path.isabs(config_file):
        # If config_file is a relative path, make it relative to GPTDB_ROOT_PATH
        config_file = os.path.join(GPTDB_ROOT_PATH, config_file)

    if not os.path.exists(config_file):
        raise FileNotFoundError(f"Configuration file not found: {config_file}")
    from gptdb.util.configure import ConfigurationManager
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="GPT-DB Webserver")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
        default=None,
        help="Path to the configuration file. Default: configs/gptdb-siliconflow.toml",
    )
