...
import gptdb_serve.datasource.serve
from gptdb.component import SystemApp
from gptdb_app.config import ApplicationConfig
...
def scan_serve_configs():
    ...
    modules = [
        "gptdb_serve.agent.chat",
        "gptdb_serve.conversation",
        "gptdb_serve.datasource",
        "gptdb_serve.gptdbs.hub",
        "gptdb_serve.gptdbs.my",
        "gptdb_serve.evaluate",
        "gptdb_serve.feedback",
        "gptdb_serve.file",
        "gptdb_serve.flow",
        "gptdb_serve.libro",
        "gptdb_serve.model",
...
def register_serve_apps(
    ...
    system_app.config.set("gptdb.app.global.language", app_config.system.language)
    global_api_keys: Optional[str] = None
    if app_config.system.api_keys:
        global_api_keys = ",".join(app_config.system.api_keys)
        system_app.config.set("gptdb.app.global.api_keys", global_api_keys)
    if app_config.system.encrypt_key:
        system_app.config.set(
            "gptdb.app.global.encrypt_key", app_config.system.encrypt_key
        )
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            PromptServe.name,
            gptdb_serve.prompt.serve.ServeConfig,
            default_user="gptdb",
            default_sys_code="gptdb",
    ...
    from gptdb_serve.conversation.serve import Serve as ConversationServe
    ...
    system_app.register(
        ...
        config=get_config(
            ...
            gptdb_serve.conversation.serve.ServeConfig,
    ...
    from gptdb_serve.flow.serve import Serve as FlowServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            FlowServe.name,
            gptdb_serve.flow.serve.ServeConfig,
    ...
    from gptdb_serve.rag.serve import Serve as RagServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            RagServe.name,
            gptdb_serve.rag.serve.ServeConfig,
    ...
    from gptdb_serve.datasource.serve import Serve as DatasourceServe
    ...
    system_app.register(
        ...
        config=get_config(
            ...
            gptdb_serve.datasource.serve.ServeConfig,
            api_keys=global_api_keys,
        ),
    )
    ...
    from gptdb_serve.feedback.serve import Serve as FeedbackServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            FeedbackServe.name,
            gptdb_serve.feedback.serve.ServeConfig,
            api_keys=global_api_keys,
        ),
    )
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            GptdbsHubServe.name,
            gptdb_serve.gptdbs.hub.serve.ServeConfig,
            api_keys=global_api_keys,
        ),
    )
    # Register serve gptdbsmy
    from gptdb_serve.gptdbs.my.serve import Serve as GptdbsMyServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            GptdbsMyServe.name,
            gptdb_serve.gptdbs.my.serve.ServeConfig,
            api_keys=global_api_keys,
        ),
    )
    # ################################ GptDbs Register End ############################
    ...
    from gptdb_serve.file.serve import Serve as FileServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            FileServe.name,
            gptdb_serve.file.serve.ServeConfig,
            host=webserver_host,
            port=webserver_port,
    ...
    from gptdb_serve.evaluate.serve import Serve as EvaluateServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            EvaluateServe.name,
            gptdb_serve.evaluate.serve.ServeConfig,
            embedding_model=llm_configs.default_embedding,
            similarity_top_k=rag_config.similarity_top_k,
            api_keys=global_api_keys,
        ),
    ...
    from gptdb_serve.libro.serve import Serve as LibroServe
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            LibroServe.name,
            gptdb_serve.libro.serve.ServeConfig,
            api_keys=global_api_keys,
        ),
    )
    ...
    system_app.register(
        ...
        config=get_config(
            serve_configs,
            ModelServe.name,
            gptdb_serve.model.serve.ServeConfig,
