...
class ApiClient:
    ...
    def _handle_response(self, response):
        if 200 <= response.status_code <= 300:
            ...
            raise Exception(
                f"Http request error, code: {response.status_code}, message: "
                f"{response.text}"
            )
...
def knowledge_delete(
    ...
    if not doc_name:
        if not confirm:
            ...
            user_input = (
                input(
                    f"Are you sure you want to delete the whole knowledge space "
                    f"{space_name}? Type 'yes' to confirm: "
                )
                .strip()
                .lower()
            )
