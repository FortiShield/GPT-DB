"""This module contains the client for the GPT-DB API."""
...
class Client:
    ...
    def __init__(
        ...
        Args:
            api_base: Optional[str], a full URL for the GPT-DB API.
                Defaults to the `http://localhost:5670/api/v2`.
            api_key: Optional[str], The gptdb api key to use for authentication.
                Defaults to None.
        ...
        .. code-block:: python
            ...
            GPTDB_API_BASE = "http://localhost:5670/api/v2"
            GPTDB_API_KEY = "gptdb"
            client = Client(api_base=GPTDB_API_BASE, api_key=GPTDB_API_KEY)
            client.chat(model="chatgpt_proxyllm", messages="Hello?")
        """
        if not api_base:
            api_base = os.getenv(
                "GPTDB_API_BASE", f"http://localhost:5670/{CLIENT_API_PATH}/{version}"
            )
        if not api_key:
            api_key = os.getenv("GPTDB_API_KEY")
