...
class FlowNodeData(BaseModel):
    ...
    id: str = Field(
        ...
        examples=[
            ...
            "resource_gptdb.model.proxy.llms.chatgpt.OpenAILLMClient_0",
        ],
    )
...
class FlowEdgeData(BaseModel):
    ...
    source: str = Field(
        ...
        examples=["resource_gptdb.model.proxy.llms.chatgpt.OpenAILLMClient_0"],
    )
...
class _VariablesRequestBase(BaseModel):
    key: str = Field(
        ...
        examples=["gptdb.model.openai.api_key"],
    )
...
class VariablesRequest(_VariablesRequestBase):
    ...
    For creating a new variable in the GPT-DB.
    """
...
class FlowPanel(BaseModel):
    ...
    source: Optional[str] = Field(
        ...
        examples=["GPT-DB-WEB", "GPTDB-GITHUB"],
    )
...
class FlowFactory:
    ...
    def pre_load_requirements(self, flow_panel: FlowPanel):
        ...
        """
        from gptdb.util.module_utils import import_from_string
