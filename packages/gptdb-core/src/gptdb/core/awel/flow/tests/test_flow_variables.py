        ...
        parameters=[
            Parameter.build_from(
                ...
                ui=ui.UIPasswordInput(
                    key="gptdb.model.openai.api_key",
                ),
            ),
            Parameter.build_from(
                "Model",
            ...
            Parameter.build_from(
                ...
                options=VariablesDynamicOptions(),
                ui=ui.UIVariablesInput(
                    key="gptdb.core.flow.params", scope=VARIABLES_SCOPE_FLOW_PRIVATE
                ),
            ),
            Parameter.build_from(
                "DAG Var 2",
...
def json_flow():
    ...
    name_to_parameters_dict = {
        "my_test_variables_operator": {
            "openai_api_key": "${gptdb.model.openai.api_key:my_key@global}",
            "model": "${gptdb.model.openai.model:default_model@global}",
            "dag_var1": "${gptdb.core.flow.params:name1@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
            "dag_var2": "${gptdb.core.flow.params:name2@%s}"
            % VARIABLES_SCOPE_FLOW_PRIVATE,
        }
    }
        ...
        (
            {
                "vars": {
                    "openai_api_key": {
                        "key": "${gptdb.model.openai.api_key:my_key@global}",
                        "value": "my_openai_api_key",
                        "value_type": "str",
                        "category": "secret",
                    },
                    "model": {
                        "key": "${gptdb.model.openai.model:default_model@global}",
                        "value": "GPT-4o",
                        "value_type": "str",
                    },
                }
...
def variables_provider():
    from gptdb_serve.flow.api.variables_provider import BuiltinFlowVariablesProvider
