...
_DEFAULT_PATH = os.path.join(
    ROOT_PATH, "packages", "gptdb-serve", "src", "gptdb_serve", "flow", "compat"
)
...
@click.option(
    ...
    required=False,
    help=_(
        "The module to scan, if not set, will scan all GPT-DB modules("
        "'gptdb,gptdb_client,gptdb_ext,gptdb_serve,gptdb_app')."
    ),
)
@click.option(
    "--output",
    ...
    help=_(
        ...
        "packages/gptdb-serve/src/gptdb_serve/flow/compat/"
    ),
)
@click.option(
    "--curr_version",
    ...
    required=False,
    help=_(
        "The current version of the flow, if not set, will read from gptdb.__version__"
    ),
)
@click.option(
...
def gen_compat(
    ...
    from gptdb.util.cli._module import _scan_awel_flow
...
def _update_template(
    ...
    from gptdb.core.awel.dag.base import DAGNode
    from gptdb.core.awel.flow.base import (
        _TYPE_REGISTRY,
    )
    from gptdb.core.awel.flow.compat import (
        FlowCompatMetadata,
    ...
    from gptdb_serve.flow.api.schemas import ServerResponse
    from gptdb_serve.flow.service.service import (
