...
GPTDB_TRACER_SPAN_ID = "GPT-DB-Trace-Span-Id"
...
def _parse_span_id(body: Any) -> Optional[str]:
    ...
    if isinstance(body, Request):
        span_id = body.headers.get(GPTDB_TRACER_SPAN_ID)
    elif isinstance(body, dict):
        span_id = body.get(GPTDB_TRACER_SPAN_ID) or body.get("span_id")
    elif isinstance(body, BaseModel):
        dict_body = model_to_dict(body)
        span_id = dict_body.get(GPTDB_TRACER_SPAN_ID) or dict_body.get("span_id")
    if not span_id:
        return None
    else:
