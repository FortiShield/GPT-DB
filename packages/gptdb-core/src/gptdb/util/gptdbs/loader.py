...
from gptdb.util.gptdbs.base import (
...
class BasePackage(BaseModel):
    ...
    def load_module_class(
        ...
        from gptdb.core.awel.dag.loader import _load_modules_from_file
...
class FlowPythonPackage(FlowPackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from gptdb.core.awel.dag.loader import _process_modules
...
class AgentPackage(BasePackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from gptdb.agent import ConversableAgent
...
class ResourcePackage(BasePackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from gptdb.agent.resource import Resource
        from gptdb.agent.resource.tool.pack import _is_function_tool
...
def parse_package_metadata(package: InstalledPackage) -> BasePackage:
    with open(
        Path(package.root) / GPTDBS_METADATA_FILE, mode="r+", encoding="utf-8"
    ) as f:
...
def _load_installed_package(path: str) -> List[InstalledPackage]:
    ...
    for package in os.listdir(path):
        ...
        gptdbs_metadata_file = full_path / GPTDBS_METADATA_FILE
        if (
            full_path.is_dir()
            and install_metadata_file.exists()
            and gptdbs_metadata_file.exists()
        ):
...
class GPTDBsLoader(BaseComponent):
    ...
    def __init__(
        ...
        load_gptdbs_interval: int = 10,
    ):
        """Initialize the GPTDBsLoader."""
        self._system_app = None
        self._install_dir = install_dir or INSTALL_DIR
        self._packages: Dict[str, BasePackage] = {}
        self._load_gptdbs_interval = load_gptdbs_interval
    ...
    def before_start(self):
        ...
        schedule.every(self._load_gptdbs_interval).seconds.do(self.load_package)
    ...
    def load_package(self, is_first: bool = False) -> None:
        ...
        try:
            ...
            if is_first:
                logger.info(
                    f"Found {len(packages)} gptdbs packages from {self._install_dir}"
                )
            for package in packages:
    ...
    def _register_packages(self, package: BasePackage):
        if package.package_type == "agent":
            from gptdb.agent import ConversableAgent, get_agent_manager
