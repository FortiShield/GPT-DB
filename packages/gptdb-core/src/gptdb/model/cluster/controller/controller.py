...
from gptdb.model.cluster.registry import EmbeddedModelRegistry, ModelRegistry
from gptdb.model.parameter import DBModelRegistryParameters, ModelControllerParameters
from gptdb.util.api_utils import APIMixin
from gptdb.util.api_utils import _api_remote as api_remote
from gptdb.util.api_utils import _sync_api_remote as sync_api_remote
...
def initialize_controller(
    ...
    if app:
        ...
        trace_file = trace_config.file or os.path.join(
            "logs", "gptdb_model_controller_tracer.jsonl"
        )
        initialize_tracer(
            trace_file,
            system_app=system_app,
            root_operation_name=trace_config.root_operation_name
            or "GPT-DB-ModelController",
...
async def api_register_instance(request: ModelInstance):
    with root_tracer.start_span(
        "gptdb.model.controller.register_instance", metadata=request.to_dict()
    ):
...
async def api_deregister_instance(
    ...
    with root_tracer.start_span(
        "gptdb.model.controller.deregister_instance", metadata=instance.to_dict()
    ):
...
def _create_registry(controller_params: ModelControllerParameters) -> ModelRegistry:
    ...
    if not controller_params.registry:
        ...
        from gptdb.datasource.rdbms.base import (
            RDBMSConnector,
            RDBMSDatasourceParameters,
        )
        from gptdb.model.cluster.registry_impl.storage import StorageModelRegistry
        from gptdb_ext.datasource.rdbms.conn_sqlite import SQLiteConnectorParameters
        ...
        if isinstance(db_config, SQLiteConnectorParameters):
            ...
            raise ValueError(
                "GPT-DB only support SQLite, MySQL and OceanBase database as metadata "
                "storage database"
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="GPT-DB API Server")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
...
def run_model_controller(config_file: str):
    from gptdb.configs.model_config import LOGDIR, ROOT_PATH
    ...
    setup_logging(
        ...
        default_logger_filename=os.path.join(LOGDIR, "gptdb_model_controller.log"),
    )
