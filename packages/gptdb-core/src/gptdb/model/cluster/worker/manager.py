...
from gptdb.model.base import ModelInstance, WorkerApplyOutput, WorkerSupportedModel
...
from gptdb.model.cluster.manager_base import (
    WorkerManager,
    WorkerManagerFactory,
    WorkerRunData,
)
from gptdb.model.cluster.registry import ModelRegistry
from gptdb.model.cluster.storage import ModelStorage, ModelStorageItem
...
from gptdb.model.utils.llm_utils import list_supported_models
from gptdb.util.fastapi import create_app, register_event_handler
...
from gptdb.util.tracer.tracer_impl import TracerParameters
...
def _create_local_model_manager(
    ...
    from gptdb.util.net_utils import _get_ip_address
    ...
    if not worker_params.register or not worker_params.controller_addr:
        ...
        from gptdb.model.cluster.controller.controller import ModelRegistryClient
...
def _build_worker(
    ...
    if worker_class:
        from gptdb.util.module_utils import import_from_checked_string
        ...
        if worker_type is None or worker_type == WorkerType.LLM:
            ...
            from gptdb.model.cluster.worker.embedding_worker import (
...
def run_worker_manager(
    ...
    **kwargs,
    ...
    setup_logging(
        ...
        default_logger_filename=os.path.join(LOGDIR, "gptdb_model_worker_manager.log"),
    )
    ...
    trace_file = trace_config.file or os.path.join(
        "logs", "gptdb_model_worker_manager_tracer.jsonl"
    )
    initialize_tracer(
        trace_file,
        system_app=system_app,
        root_operation_name=trace_config.root_operation_name or "GPT-DB-ModelWorker",
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="GPT-DB Model Worker")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
