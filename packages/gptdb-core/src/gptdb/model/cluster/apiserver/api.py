...
from gptdb.model.parameter import ModelAPIServerParameters, WorkerType
from gptdb.util.chat_util import transform_to_sse
from gptdb.util.fastapi import create_app
from gptdb.util.tracer import initialize_tracer, root_tracer, trace
...
class APIServer(BaseComponent):
    ...
    async def get_available_models(self) -> ModelList:
        ...
        for m in models:
            model_cards.append(
                ModelCard(
                    id=m, root=m, owned_by="GPT-DB", permission=[ModelPermission()]
                )
            )
    ...
    async def embeddings_generate(
        ...
        with root_tracer.start_span(
            "gptdb.model.apiserver.generate_embeddings",
...
async def create_chat_completion(
    ...
    trace_kwargs = {
        "operation_name": "gptdb.model.apiserver.create_chat_completion",
        "metadata": {
            "model": request.model,
...
async def create_completion(
    ...
    trace_kwargs = {
        "operation_name": "gptdb.model.apiserver.create_completion",
...
async def create_relevance(
    ...
    with root_tracer.start_span(
        "gptdb.model.apiserver.generate_relevance",
        metadata={
            "model": request.model,
            "query": request.query,
            "worker_type": WorkerType.RERANKER.value,
...
def _initialize_all(controller_addr: str, system_app: SystemApp):
    ...
    from gptdb.model.cluster.worker.manager import _DefaultWorkerManagerFactory
...
def initialize_apiserver(
    ...
    setup_logging(
        ...
        default_logger_filename=os.path.join(LOGDIR, "gptdb_model_apiserver.log"),
    )

    trace_file = trace_config.file or os.path.join(
        "logs", "gptdb_model_apiserver_tracer.jsonl"
    )
    initialize_tracer(
        trace_file,
        system_app=system_app,
        root_operation_name=trace_config.root_operation_name or "GPT-DB-APIServer",
