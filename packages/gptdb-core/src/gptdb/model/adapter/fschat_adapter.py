...
from gptdb.model.adapter.base import LLMModelAdapter
from gptdb.model.adapter.template import ConversationAdapter, PromptType
...
# If some model is not in the blacklist, but it still affects the loading of GPT-DB,
...
class FastChatLLMModelAdapterWrapper(LLMModelAdapter):
    ...
    def get_generate_stream_function(
        ...
        if _IS_BENCHMARK:
            from gptdb.util.benchmarks.llm.fastchat_benchmarks_inference import (
                generate_stream,
            )
