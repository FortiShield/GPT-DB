...
async def get_nodes(
    ...
    """
    from gptdb.core.awel.flow.base import _OPERATOR_REGISTRY
...
async def export_flow(
    uid: str,
    export_type: Literal["json", "gptdbs"] = Query(
        "json", description="export type(json or gptdbs)"
    ),
    ...
    elif export_type == "gptdbs":
        from ..service.share_utils import _generate_gptdbs_zip

        if format == "json":
            raise HTTPException(
                status_code=400, detail="json response is not supported for gptdbs"
            )

        zip_buffer = await blocking_func_to_async(
            global_system_app, _generate_gptdbs_zip, package_name, flow
        )
        return StreamingResponse(
            zip_buffer,
