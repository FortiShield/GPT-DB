...
from gptdb.core.awel.trigger.http_trigger import CommonLLMHttpTrigger
from gptdb.core.awel.util.chat_util import (
...
from gptdb_serve.core import BaseService, blocking_func_to_async
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def __init__(
        ...
        self._gptdbs_loader: Optional[GPTDBsLoader] = None
    ...
    def init_app(self, system_app: SystemApp) -> None:
        ...
        self._gptdbs_loader = system_app.get_component(
            GPTDBsLoader.name,
            GPTDBsLoader,
            or_register_component=GPTDBsLoader,
            load_gptdbs_interval=self._serve_config.load_gptdbs_interval,
        )
    ...
    def before_start(self):
        ...
        self._pre_load_dag_from_gptdbs()

    def after_start(self):
        """Execute after the application starts"""
        self.load_dag_from_db()
        self.load_dag_from_gptdbs(is_first_load=True)
        schedule.every(self._serve_config.load_gptdbs_interval).seconds.do(
            self.load_dag_from_gptdbs
        )
    ...
    @property
    def gptdbs_loader(self) -> GPTDBsLoader:
        """Returns the internal GPTDBsLoader."""
        if self._gptdbs_loader is None:
            raise ValueError("GPTDBsLoader is not initialized")
        return self._gptdbs_loader
    ...
    def _pre_load_dag_from_gptdbs(self):
        """Pre load DAG from gptdbs"""
        flows = self.gptdbs_loader.get_flows()
        for flow in flows:
            try:
                ...
                logger.warning(
                    ...
                    f"gptdbs error: {str(e)}"
                )

    def load_dag_from_gptdbs(self, is_first_load: bool = False):
        """Load DAG from gptdbs"""
        flows = self.gptdbs_loader.get_flows()
        for flow in flows:
            try:
                ...
                logger.warning(
                    f"Load DAG {flow.name} from gptdbs error: {str(e)}, detail: "
                    f"{message}"
                )
    ...
    def _parse_flow_category(self, dag: DAG) -> FlowCategory:
        ...
        """
        from gptdb.core.awel.flow.base import _get_type_cls
    ...
    async def debug_flow(
        ...
        Returns:
            AsyncIterator[ModelOutput]: The output
        """
        from gptdb.core.awel.dag.dag_manager import _parse_metadata
