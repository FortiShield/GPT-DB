...
from gptdb.configs.model_config import BENCHMARK_DATA_ROOT_PATH
...
from gptdb.util import PaginationResult, get_or_create_event_loop
from gptdb.util.benchmarks import StorageUtil
from gptdb_serve.evaluate.service.benchmark.task.benchmark_agent_task import (
    BenchmarkAgentTask,
)
from gptdb_serve.evaluate.service.benchmark.task.benchmark_llm_task import (
    BenchmarkLLMTask,
)
...
BENCHMARK_SERVICE_COMPONENT_NAME = "gptdb_serve_evaluate_benchmark_service"

STANDARD_BENCHMARK_FILE_PATH = "https://github.com/khulnasoft/Falcon"

BENCHMARK_OUTPUT_RESULT_PATH = os.path.join(BENCHMARK_DATA_ROOT_PATH, "result")


def get_rag_service(system_app) -> RagService:
    return system_app.get_component("gptdb_rag_service", RagService)


def get_prompt_service(system_app) -> PromptService:
    return system_app.get_component("gptdb_serve_prompt_service", PromptService)
