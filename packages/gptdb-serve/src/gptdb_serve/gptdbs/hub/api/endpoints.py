...
async def install(request: ServeRequest):
    logger.info(f"gptdbs install:{request.name},{request.type}")

    try:
        from gptdb_serve.gptdbs.my.config import (
            SERVE_SERVICE_COMPONENT_NAME as MY_GPTS_SERVICE_COMPONENT,
        )
        from gptdb_serve.gptdbs.my.service.service import Service as MyGptsService
        ...
        await blocking_func_to_async(
            ...
            gptdb_path=request.storage_url,
            user_name=None,
            sys_code=None,
        )
        return Result.succ(None)
