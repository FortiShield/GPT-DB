...
class BaseServe(BaseComponent, ABC):
    """Base serve component for GPT-DB"""

    name = "gptdb_serve_base"
    ...
    def create_or_get_db_manager(self) -> DatabaseManager:
        ...
        """
        from gptdb.storage.metadata import Model, UnifiedDBManagerFactory, db
    ...
    def call_on_current_serve(
        ...
        Return default_value if the serve component is not exist or the function return
        None.

        Args:
            system_app (SystemApp): The system app
            func (Callable[[BaseServe], Any]): The function to call
            default_value (Optional[Any], optional): The default value. Defaults to
                None.
