...
from gptdb_serve.agent.app.gpts_server import available_llms
...
@router.get("/v1/gptdbs/list", response_model=Result[List[GptsApp]])
async def get_gptdbs(user_code: str = None, sys_code: str = None):
    logger.info(f"get_gptdbs:{user_code},{sys_code}")
    try:
        ...
        logger.error(f"get_gptdbs failed:{str(e)}")
