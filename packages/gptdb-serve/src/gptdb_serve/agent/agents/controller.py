...
from gptdb.agent.core.memory.gpts import GptsMessage
from gptdb.agent.core.schema import Status
from gptdb.agent.resource import get_resource_manager
from gptdb.agent.util.llm.llm import LLMStrategyType
...
from gptdb.core.interface.message import StorageConversation
...
from gptdb_app.gptdb_server import system_app
from gptdb_app.scene.base import ChatScene
from gptdb_serve.conversation.serve import Serve as ConversationServe
from gptdb_serve.core import blocking_func_to_async
from gptdb_serve.prompt.api.endpoints import get_service
from gptdb_serve.prompt.service import service as PromptService
...
class MultiAgents(BaseComponent, ABC):
    ...
    def after_start(self):
        from gptdb_serve.agent.app.controller import gpts_dao

        gpts_dao.init_native_apps()

        gpts_dao.init_native_apps("gptdb")
    ...
    def get_or_build_agent_memory(self, conv_id: str, gptdbs_name: str) -> AgentMemory:
        from gptdb.rag.embedding.embedding_factory import EmbeddingFactory
        from gptdb_serve.rag.storage_manager import StorageManager
        ...
        **ext_info,
        ...
        if gpts_conversations and len(gpts_conversations) > 0:
            ...
            if last_gpts_conversation.state == Status.WAITING.value:
                ...
                from gptdb_serve.agent.agents.expand.app_start_assisant_agent import (
        ...
        #     )
        #     from gptdb_app.openapi.api_v1.api_v1 import get_chat_flow
